...
printExp(Indent, handlexp(E,ML)) :- printExp(Indent, E), nl, print(Indent), print('handle'), nl,
                                    printMatchList(ML).
printExp(Indent, caseof(E,ML)) :- print('case'), printExp(Indent, E), print('of'), printMatchList(ML).                                                                  
...
printPat(A,[],_) :- nl,nl, print('Typechecker Error: Unknown pattern '),print(A),nl,nl, throw(error('unknown pattern')).
...
typecheckMatch(Env,Id,match(Pat,Exp)) :- find(Env,Id,fn(A,B)), typePat(Pat,Epati,A), append(Epati,Env,Re), typecheck(Re,Exp,B).
...
typeListPats([],[],_).
typeListPats([H|T],Env,A):- typePat(H,HEnv,A), typeListPats(T,TEnv,A), append(HEnv,TEnv,Env).
...
typePat(boolpat(_),[],bool):- !. 
typePat(numpat(_),[],int):- !. 
typePat(tuplepat(L),Env,tuple(T)) :- typePats(L,Env,T), !.
typePat(listpat(L),Env,listOf(A)):- typeListPats(L,Env,A), !.
...
typecheck(Env,caseof(Exp1,Matches), T) :- typecheck(Env,Exp1,bool), typecheckMatches([('case',fn(exn,T))|Env],'case',Matches), !.

typecheck(Env,caseof(Exp1,Matches), _) :- typecheck(Env,Exp1,T), typecheckMatches([('case',fn(exn,S))|Env],'case',Matches),
                                            T \= S, print('Error: Expression and case matches must be of the same type.'), nl,
                                            print('Expression type was '), printType(T, _), print(' and case match type is '), 
                                            printType(S,_), nl, throw(typeerror('expression and case match typemismatch')).
...
typecheck(Env,whiledo(Exp1,Exp2), RT) :- typecheck(Env,Exp1,bool), typecheck(Env,Exp2,RT),!. 

typecheck(Env,whiledo(Exp1,Exp2), _) :- typecheck(Env,Exp1,bool), typecheck(Env,Exp2,DoType), 
                                                    print('Error: Result types of while and do expressions must match.'), nl,
                                                    print('While Expression type is: '), printType(bool,_), nl,
                                                    print('Do Expression type is: '), printType(DoType,_), nl, 
                                                    throw(typeerror('result type mismatch in while-do expression')).

typecheck(Env,whiledo(Exp1,_), _) :- typecheck(Env,Exp1,Exp1Type), Exp1Type \= bool,
                                                    print('Error: Condition of while-do expression must have bool type.'), nl,
                                                    print('Condition Expression type was: '), printType(Exp1Type,_), nl,     
                                                    throw(typeerror('type not bool in while-do expression condition')).                                                 
...
typecheckSequence(_,[],tuple([])).
typecheckSequence(Env,[X],Type):- typecheck(Env,X,Type).
typecheckSequence(Env,[H|T],Type):- typecheck(Env,H,_), typecheckSequence(Env,T,Type).
