    ...
    (* The constants function is responsible for returning a list of all literal values in a program. 
     ...
     fun constants(ast) = 
         let fun decconsts(bindval(_,exp)) = con exp
             ...
             and con(num(i)) = [i]
               ...
               | con(whiledo(e1,e2)) = (con e1) @ (con e2)
     ...
     and localBindings(ast,pats,globalBindings,scope) = 
         let val freeVars = ref []
             ...
             fun bindingsOf(num(n),bindings,scope) = ()
               ...
               | bindingsOf(ifthen(e1,e2,e3),bindings,scope) = (
           		      bindingsOf(e1,bindings,scope);
           		      bindingsOf(e2,bindings,scope);
           		      bindingsOf(e3,bindings,scope)
               )

               | bindingsOf(whiledo(e1,e2),bindings,scope) = (
                    bindingsOf(e1,bindings,scope);
                    bindingsOf(e2,bindings,scope)
               )
     ...
     fun codegen(num(i),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = 
        ...
        | codegen(infixexp(":=",id(name),t2),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) =
          let val _ = codegen(t2,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)
            val noneIndex = lookupIndex("None",consts)
          in
       ...
       | codegen(negate(t),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) =
          let val _ =codegen(num("0"),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)
       ...
       | codegen(ifthen(t1,t2,t3),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) =
         ...
         in
           ...
           TextIO.output(outFile,indent^"LOAD_FAST 1\n");
     ...
     and nestedfuns(ast,outFile,indent,globals,env,globalBindings,scope) =
	       let fun functions(num(n)) = ()
               ...
               | functions(whiledo(e1, e2)) = (functions e1;functions e2)
     ...
     fun writeTerm(outFile,ast) = 
         let fun print(s) = TextIO.output(outFile,s)
             ...
             fun writeExp(indent,num(i)) = print("num("^i^")")
               ...
               | writeExp(indent,ifthen(e1,e2,e3)) = 
     ...
     (* Here is where it all begins. The file is parsed to produce an abstract syntax tree. Then the output file a.casm is opened for writing. Constants
        ...
        that would mean that we could not compile the code completely. The FUNCTION keyword is written to the file. Then nestedfunctions is called to 
